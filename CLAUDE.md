...
- **A static property of the boot rides the REGISTER message, not the stats tick, and is PERSISTED as well as held live.** `base_os` (`"emos"`/`"fireos"`, absent on older firmware) says which userspace the device booted, and the controller gates Android-only payloads on it — the debloat script and the pm-hide list mean nothing without a package manager. It shipped on the stats report for exactly one commit and that was the bug: its only consumer is `reconcile_on_connect`, which runs the instant a device connects, ~30s before the first stats tick. So it read as unknown precisely when it was asked, the Magisk `service.d` script was pushed at an emOS device, and each attempt sat out the full 120s transfer timeout — 240s across two attempts, measured on EFF 2026-09-04, presenting as "the OTA timed out" when the OTA was healthy. Absence resolves to Android, so the existing fleet is untouched. The rule generalises: ask when the consumer needs the answer, not when it is convenient to send.
  **`emos_ver` is the same field's companion and repeats both halves of the
  rule.** It says WHICH emOS the device booted — `VERSION_ID` out of the same
  `/etc/os-release` that `base_os` is detected from, so it costs one extra
  parse of a file already open — and it is stored for v21's reason. The
  motivation is worth keeping: reading it off a device costs a ~26s shell
  probe, which is affordable for a tab somebody deliberately opened and
  impossible for a list, so the emOS version was invisible to every page that
  could have told somebody an update was waiting (#255). Empty on FireOS,
  absent on older firmware, and neither may render as up to date — `base_os`
  beside it is what tells "no emOS here" from "an emOS nobody can read".

  **It is never written as NULL**, which is the one asymmetry `base_os` does
  not have: `base_os` is corrected on every register, so a stale version next
  to a corrected base is harmless, while clearing it whenever a FireOS device
  registers would throw away the last thing known about a device that is
  merely on old firmware today.
...
**`bassGuardJackBypass` is an EIGHTH key in that family and the only one the controller cannot mirror**, which is worth stating rather than leaving as an omission somebody later "completes". It turns the bass guard off while a plug is in the headphone jack (#231) — the guard is Amazon's voicing for the 1.5" internal driver, measured at −28.6dB at 31.5Hz, and the jack's switch contacts take that driver out of the path physically, so into an amplifier the protection is only a low-cut. Resolving it needs the plug position, **no device reports the plug position to the controller**, and the controller-side chain therefore could not honour it however faithfully it was mirrored — so the mirror is absent on purpose and `tests/test_jack_bypass_wiring.py` pins the absence. The dashboard offers it only for firmware announcing BOTH `output_chain` and `jack_detect`, disabled with the reason otherwise, because either half alone is a control that saves into a key nothing reads.

It defaults **OFF**, and the default is the considered part rather than caution for its own sake: `device/CLAUDE.md` records `Ext_Speaker_Amp_Switch` observed `Off` while the internal speaker was audibly playing, and that observation has not been retested since the jack gain was fixed. If the control does not gate the internal driver, an automatic bypass would send unguarded bass to a 1.5" driver on every device in the fleet on the strength of an untested assumption. A setting costs the one user with a hi-fi one toggle and costs everybody else nothing — and turning it on IS most of the retest, which is why the dashboard's own note says to try it with the external speakers switched off.
