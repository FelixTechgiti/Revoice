...
- **`Ext_Speaker_Amp_Switch` was observed `Off` while the internal speaker
  ...
  **That untested observation is also why the output chain's jack bypass is
  a SETTING and defaults off** (#231). The bass guard is voiced for the 1.5"
  internal driver and removes essentially everything below 160Hz — measured
  −28.6dB at 31.5Hz, −19.0dB at 63Hz, with the limiter contributing nothing
  at −12dBFS — so with a plug in it is guarding a load that is not connected,
  and the result is the "kein dynamischer Umfang, nur Mitteltöne" this was
  reported as. Bypassing it automatically is the obvious fix and is exactly
  what the observation above forbids: if ctl 5 does not gate the internal
  driver, that sends unguarded bass to it on every device in the fleet.
  `bassGuardJackBypass` gives the one user with an amplifier the lever and
  leaves everybody else where they were — and turning it on with the external
  speakers switched off IS the cheap test named above, where bass audible from
  the Dot itself is the answer.

  The resolution lives in `outchain.Params.ForJack` (untagged, host-tested);
  `PcmSpeaker.applyOutputChainParams` is the only place the chain is handed
  parameters, reached from `SetOutputChain` and from `SetJackRouting`. Both
  paths are needed and neither is obvious on its own: nothing pushes config
  when a plug moves, and nothing re-reads the plug when config is pushed. An
  UNKNOWN plug position resolves to "not inserted", so the guard stays on
  until `jack.Watch` has dispatched — the same conservative answer
  `jack.Inserted()` gives for absent hardware. The wiring is pinned by a
  source guard (`outputchain_guard_test.go`), because both call sites are
  `//go:build server` and a host `go test ./...` compiles neither.

  The capability is `jack_detect`, announced only when the switch is actually
  readable, for the reason `ambient_light` is: whether a jack exists is a
  property of the BOARD. The dashboard needs it together with `output_chain`,
  since the controller can supply neither half — no device reports the plug
  position to it, so its own copy of the chain can never implement this.
