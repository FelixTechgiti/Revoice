	...
	for {
		...
		if usingStatic {
			...
			if server != nil && probeTCP(server.Addr, 3*time.Second) {
				...
				// Which of the two failed is the whole diagnosis, so the
				// record says it. A remembered address that does not answer
				// while the device holds an IP is a UNICAST failure — the
				// network, not multicast; no remembered address at all, or
				// one that answers while nothing else works, points the other
				// way. Without this the log says "no controller" for both.
				if server == nil {
				...
				// length of the outage, and it used to: FindServer browses
				// and nothing else. The moment the probe is guaranteed to
				// fail is a controller RESTART — its listener is down for a
				// container restart and back seconds later — so the failure
				// that costs the most is also the most recoverable one.
				// Re-tested every round; see discovery.FindServerWith.
				remembered := server
		...
		// The all-clear rides `healthy` rather than a registration count, and
		// upstream's flag is the better signal for it: it is set at the read
		// loop's exit, the one place a connection that LASTED can be told
		// from one that was accepted and dropped. An outage report a flap
		// cancels is worse than none, because it says the problem went away.
		if healthy {
