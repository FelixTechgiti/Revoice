        ...
        while True:
            ...
            while len(buf) >= CHUNK_BYTES:
                ...
                if fired:
                    ...
                    # _reachable_count, not len(_devices): a device inside a
                    # control-plane reconnect grace is held in the registry
                    # and cannot answer (#354), so counting it as contention
                    # taxes every barge with the ~364ms window the shortcut
                    # exists to skip. Same rule as the wake path below.
...
async def handle_control(ws: WebSocketServerProtocol, secure: bool = False):
    ...
    try:
        ...
        # the same rule the emOS version above follows, arrived at
        # independently upstream, which is worth noticing rather than merging
        # away: two fields on one message, both "store only a real answer".
