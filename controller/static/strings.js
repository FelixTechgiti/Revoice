...
(function (global) {
  ...
  const STRINGS = {
    en: {
      ...
      // -- Provisioning wizard: steps and connect advice --
      wizStepConnectDevice: 'Connect Device',
      wizStepConnectTwrp: 'Connect to TWRP',
      wizStepPatchBoot: 'Patch Boot Image',
      wizStepInstallMagisk: 'Install Magisk',
      wizStepPreseedDb: 'Pre-seed Root DB',
      wizStepReboot: 'Reboot to Android',
      wizStepReconnect: 'Reconnect',
      wizStepVerifyRoot: 'Verify Root',
      wizStepDisableAlexa: 'Disable Alexa',
      wizStepDebloat: 'Debloat',
      wizStepWifi: 'Configure WiFi',
      wizStepInstallEm: 'Install Revoice',
      wizStepInstallOww: 'Wake Word Assets',
      wizStepEscrowBoot: 'Escrow Boot Image',
      wizStepBuildEmos: 'Build emOS',
      wizStepFlashEmos: 'Flash and Verify',
      wizStepRebootWatch: 'Reboot and Watch',
      wizDescConnectDevice: 'Connect the Echo Dot via USB. Device should be on and booted into Android. Appears as "AEOBC" in the USB picker. Already running emOS? It has no adbd, so the picker stays empty — run /init recovery from its console first, then connect here.',
      wizDescConnectTwrp: 'Wait for TWRP recovery to appear, then reconnect. Appears as "Echo" in the USB picker.',
      wizDescPatchBoot: 'Apply SELinux permissive patch and add init.rc service entries.',
      wizDescInstallMagisk: 'Flash Magisk 17.3 for persistent root access.',
      wizDescPreseedDb: 'Grant root to ADB shell without a screen prompt.',
      wizDescReboot: 'Reboot device to Android.',
      wizDescReconnect: 'Re-connect ADB as soon as the device appears as "AEOBC" in the USB picker — no need to wait for it to finish booting, the next step does that.',
      wizDescVerifyRoot: 'Confirm Magisk root is working.',
      wizDescDisableAlexa: 'Silence the Amazon setup assistant and disable the Alexa voice pipeline, before the device ever reaches WiFi.',
      wizDescDebloat: 'Hide non-essential Amazon packages and stop background daemons (~130MB RAM freed).',
      wizDescWifi: 'Connect the device to your local WiFi network.',
      wizDescInstallEm: 'Push server binary and startup script to device.',
      wizDescInstallOww: 'Push the ONNX runtime and wake models (~15MB) used for on-device wake word detection.',
      wizDescConnectTwrpEmos: 'Wait for TWRP recovery to appear, then reconnect. Appears as "Echo" in the USB picker. Everything after this happens here.',
      wizDescEscrowBoot: 'Read the stock boot partition off the device and keep a copy. This one file is both the build input and the ten-second undo.',
      wizDescInstallEmEmos: 'Push the server binary, startup script and TLS credentials to /data, which survives the boot-partition write.',
      wizDescBuildEmos: 'The controller repacks your own escrowed image with the emOS init, reusing your kernel and device trees.',
      wizDescFlashEmos: 'Write the built image to the boot partition and read it back to confirm it landed.',
      wizDescRebootWatch: 'Reboot into emOS and follow the first boot over the USB serial console while the ring fills.',
      wizDescWifiRegister: 'Scan and join a network using the device’s own radio, then wait for it to register with the controller.',
      wizAdviceNothingSelected: 'Nothing was selected — and two different situations look identical here.',
      wizAdviceDismissed: '· The picker was dismissed, or the Dot is off or on a charge-only cable. Replug and retry.',
      wizAdviceAlreadyEmos: '· The Dot is ALREADY RUNNING emOS. Retrying cannot help: emOS has no adbd and never will, so it offers a serial console and the ADB picker has nothing to show.',
      wizAdviceInitRecovery: 'If it is on emOS: open the console and run `/init recovery`. The Dot reboots into TWRP, and this step accepts a device that is already there — it reads the FireOS build off /system rather than trusting the recovery ramdisk’s own properties.',
      // -- Provisioning wizard --
      wizTitle: 'Provision Echo Dot',
      wizSubtitle: 'Chrome/Edge only · USB-A cable · amonet-biscuit prerequisite',
      wizUsbBlocked: 'USB is unavailable in this browser',
      wizWhatToInstall: 'What to install',
      wizDefault: 'default',
      wizEmosSub: 'No Amazon software. The 3.5mm jack works.',
      wizEmosWarn: 'Newer, and going back to FireOS wipes the device.',
      wizFireosName: 'FireOS + root',
      wizFireosSub: 'Keeps Android. Every device in the field runs this.',
      wizFireosWarn: 'The 3.5mm jack is unreliable on this path.',
      wizUpNext: 'Up next',
      wizConnectDeviceBtn: 'Connect Device',
      wizConnectTwrpBtn: 'Connect to TWRP',
      wizReconnectDeviceBtn: 'Reconnect Device',
      wizRebootToAndroidBtn: 'Reboot to Android',
      wizPickDifferentFile: 'SELECT A DIFFERENT FILE',
      wizMagiskZip: 'MAGISK-V17.3.ZIP',
      wizFlashMagisk: 'Flash Magisk',
      wizInstallLatest: 'Install latest from GitHub',
      wizChecking: 'Checking…',
      wizCheckNewer: 'Check for newer release',
      wizOr: '— or —',
      wizPickDifferentBuild: 'SELECT A DIFFERENT BUILD (ARMv7)',
      wizCustomBinary: 'CUSTOM REVOICE SERVER BINARY (ARMv7)',
      wizInstallCustomBuild: 'Install Custom Build',
      wizBuildLatestEmos: 'Build with the latest emOS release',
      wizPickDifferentInit: 'SELECT A DIFFERENT INIT',
      wizCustomInit: 'YOUR OWN INIT BINARY (AARCH64, STATIC)',
      wizBuildWithInit: 'Build with this init',
      wizFlashEmosBtn: 'Flash emOS',
      wizBootPartition: 'the boot partition',
      wizNoEscrow: 'This session has no escrowed image — choose the revoice-stock-boot-*.img downloaded at step 3.',
      wizRestoreEscrow: 'Restore escrowed boot image',
      wizRingHeading: 'Watch the light ring on this boot',
      wizRingFillingBold: 'Filling, then white, then fading',
      wizRingFillingRest: 'up and on the network. Done, about 30 seconds.',
      wizRingTwoSegBold: 'Two lit segments at the top, throbbing',
      wizRingTwoSegRest: 'waiting for the network. Normal, and most of the boot.',
      wizRingAmberBold: 'Solid amber',
      wizRingAmberRest: 'the device is restoring its own last good image. Leave it alone; it reboots itself.',
      wizRingRedBold: 'Red and stopped',
      wizRingRedRest: 'a boot stage failed. Recoverable, see below.',
      wizRingOrbitBold: 'A single segment orbiting a full blue ring, for more than a minute',
      wizRingOrbitRest: 'emOS never started. This is the one that needs you.',
      wizNoPowerCycleBold: 'If it does not come up, do not keep power cycling it.',
      wizMuteButton: 'mute',
      wizCyanPattern: 'alternating cyan pattern',
      wizRebootConnectConsole: 'Reboot and Connect Console',
      wizCancelStep: 'Cancel step',
      wizStepFailed: 'This step failed',
      wizRetry: 'Retry',
      wizReconnect: 'Reconnect',
      wizReconnectDevice: 'Reconnect device',
      wizDownloadDiagnostics: 'Download diagnostics',
      wizComplete: 'Provisioning complete. The device has rebooted and will discover the controller via mDNS, appearing in the dashboard as a pending device within ~30s.',
      wizDone: 'Done',
      wizOutput: 'Output',
      wizCopyLog: 'Copy log',
      wizNoOutput: '— no output yet —',
      wizFlashBlurb(target) {
        return `Writes the built image to ${target}. Your escrowed image `
             + 'restores it in about ten seconds and leaves /data alone.';
      },
      wizRestoreBlurb(target, md5) {
        return `Put ${target} back to the image escrowed at step 3${md5}. `
             + 'Verified against the partition afterwards, and /data is '
             + 'untouched \u2014 everything installed stays.';
      },
      // The recovery route, kept whole because it asks somebody to act on
      // their own hardware — CLAUDE.md's one exception to keeping user-facing
      // text short. `mute` and the cyan pattern are the two things they have
      // to recognise, so they ride as their own keys and stay bold.
      wizNoPowerCyclePre: 'To reach TWRP: unplug the power, hold the ',
      wizNoPowerCycleMid: ' button down, and apply power with it still held \u2014 the ring shows an ',
      wizNoPowerCyclePost: ' once you are in recovery. Reconnect here and use ',
      wizNoPowerCycleEnd: ' below: about ten seconds, and it leaves everything on /data alone. Repeatedly power cycling a device that will not boot is what turns a recoverable one into a case-opening job.',
      wizLatestOnGithub(v) { return `Latest on GitHub: ${v}`; },
      wizOutputLines(n) { return ` \u2014 ${n} lines`; },
      wizMigrateDevice(id) { return `Migrate "${id}" to emOS (keep its settings)`; },
      wizDeleteDevice(id) { return `Delete "${id}" from controller`; },
      // -- WiFi panel --
      wifiScanForNetworks: 'Scan for networks',
      wifiSsidPlaceholder: 'Select above or type network name',
      wifiPassword: 'PASSWORD',
      wifiPassphrasePlaceholder: 'WPA passphrase',
      wifiHide: 'hide',
      wifiShow: 'show',
      wifiConnect: 'Connect',
      wifiSkip: 'Skip (already connected)',
      wifiAbort: 'Abort provisioning',
      wifiNetworksFound(n) { return `${n} network${n !== 1 ? 's' : ''} found`; },
    ...
    de: {
      ...
      // -- Provisioning wizard: steps and connect advice --
      wizStepConnectDevice: 'Gerät verbinden',
      wizStepConnectTwrp: 'Mit TWRP verbinden',
      wizStepPatchBoot: 'Boot-Image patchen',
      wizStepInstallMagisk: 'Magisk installieren',
      wizStepPreseedDb: 'Root-Datenbank vorbefüllen',
      wizStepReboot: 'Nach Android neu starten',
      wizStepReconnect: 'Neu verbinden',
      wizStepVerifyRoot: 'Root prüfen',
      wizStepDisableAlexa: 'Alexa abschalten',
      wizStepDebloat: 'Entrümpeln',
      wizStepWifi: 'WLAN einrichten',
      wizStepInstallEm: 'Revoice installieren',
      wizStepInstallOww: 'Wachwort-Dateien',
      wizStepEscrowBoot: 'Boot-Image sichern',
      wizStepBuildEmos: 'emOS bauen',
      wizStepFlashEmos: 'Flashen und prüfen',
      wizStepRebootWatch: 'Neu starten und zusehen',
      wizDescConnectDevice: 'Den Echo Dot per USB anschließen. Das Gerät soll an und in Android gestartet sein. Es erscheint als „AEOBC" in der USB-Auswahl. Läuft darauf schon emOS? Dann hat es kein adbd, die Auswahl bleibt also leer — erst /init recovery über dessen Konsole ausführen, dann hier verbinden.',
      wizDescConnectTwrp: 'Warten, bis die TWRP-Recovery erscheint, dann neu verbinden. Sie erscheint als „Echo" in der USB-Auswahl.',
      wizDescPatchBoot: 'Den SELinux-Permissive-Patch anwenden und die Diensteinträge in init.rc ergänzen.',
      wizDescInstallMagisk: 'Magisk 17.3 flashen, damit Root dauerhaft bleibt.',
      wizDescPreseedDb: 'Der ADB-Shell Root geben, ohne Nachfrage auf einem Bildschirm.',
      wizDescReboot: 'Das Gerät nach Android neu starten.',
      wizDescReconnect: 'ADB neu verbinden, sobald das Gerät als „AEOBC" in der USB-Auswahl auftaucht — auf das Ende des Bootvorgangs muss man nicht warten, das macht der nächste Schritt.',
      wizDescVerifyRoot: 'Bestätigen, dass Magisk-Root funktioniert.',
      wizDescDisableAlexa: 'Den Amazon-Einrichtungsassistenten stummschalten und die Alexa-Sprachkette abschalten, bevor das Gerät überhaupt ins WLAN kommt.',
      wizDescDebloat: 'Nicht benötigte Amazon-Pakete ausblenden und Hintergrunddienste stoppen (~130 MB RAM frei).',
      wizDescWifi: 'Das Gerät mit deinem WLAN verbinden.',
      wizDescInstallEm: 'Die Server-Binärdatei und das Startskript auf das Gerät schieben.',
      wizDescInstallOww: 'Die ONNX-Laufzeit und die Wachwortmodelle (~15 MB) aufspielen, die für die Wachworterkennung auf dem Gerät gebraucht werden.',
      wizDescConnectTwrpEmos: 'Warten, bis die TWRP-Recovery erscheint, dann neu verbinden. Sie erscheint als „Echo" in der USB-Auswahl. Alles Weitere passiert dort.',
      wizDescEscrowBoot: 'Die originale Boot-Partition vom Gerät lesen und eine Kopie behalten. Diese eine Datei ist zugleich die Bauvorlage und das Zurück in zehn Sekunden.',
      wizDescInstallEmEmos: 'Die Server-Binärdatei, das Startskript und die TLS-Zugangsdaten nach /data schieben — das übersteht das Schreiben der Boot-Partition.',
      wizDescBuildEmos: 'Der Controller packt dein eigenes gesichertes Image mit dem emOS-init neu — mit deinem Kernel und deinen Device-Trees.',
      wizDescFlashEmos: 'Das gebaute Image in die Boot-Partition schreiben und zur Kontrolle wieder auslesen.',
      wizDescRebootWatch: 'Nach emOS neu starten und den ersten Boot über die serielle USB-Konsole verfolgen, während sich der Ring füllt.',
      wizDescWifiRegister: 'Mit dem Funkmodul des Geräts ein Netz suchen und beitreten, dann warten, bis es sich beim Controller anmeldet.',
      wizAdviceNothingSelected: 'Es wurde nichts ausgewählt — und zwei völlig verschiedene Lagen sehen hier gleich aus.',
      wizAdviceDismissed: '· Die Auswahl wurde weggeklickt, oder der Dot ist aus oder hängt an einem reinen Ladekabel. Neu einstecken und erneut versuchen.',
      wizAdviceAlreadyEmos: '· Auf dem Dot läuft BEREITS emOS. Ein erneuter Versuch kann nicht helfen: emOS hat kein adbd und wird nie eines haben, es bietet eine serielle Konsole an und die ADB-Auswahl hat nichts zu zeigen.',
      wizAdviceInitRecovery: 'Wenn emOS läuft: die Konsole öffnen und `/init recovery` ausführen. Der Dot startet in TWRP, und dieser Schritt akzeptiert ein Gerät, das schon dort ist — er liest den FireOS-Build von /system, statt den Eigenschaften der Recovery-Ramdisk zu trauen.',
      // -- Provisioning wizard --
      wizTitle: 'Echo Dot einrichten',
      wizSubtitle: 'nur Chrome/Edge · USB-A-Kabel · amonet-biscuit vorausgesetzt',
      wizUsbBlocked: 'USB steht in diesem Browser nicht zur Verfügung',
      wizWhatToInstall: 'Was installiert wird',
      wizDefault: 'Standard',
      wizEmosSub: 'Keine Amazon-Software. Die 3,5-mm-Buchse funktioniert.',
      wizEmosWarn: 'Neuer — und der Weg zurück zu FireOS löscht das Gerät.',
      wizFireosName: 'FireOS + Root',
      wizFireosSub: 'Behält Android. Jedes Gerät im Feld läuft so.',
      wizFireosWarn: 'Die 3,5-mm-Buchse ist auf diesem Weg unzuverlässig.',
      wizUpNext: 'Als Nächstes',
      wizConnectDeviceBtn: 'Gerät verbinden',
      wizConnectTwrpBtn: 'Mit TWRP verbinden',
      wizReconnectDeviceBtn: 'Gerät neu verbinden',
      wizRebootToAndroidBtn: 'Nach Android neu starten',
      wizPickDifferentFile: 'ANDERE DATEI WÄHLEN',
      wizMagiskZip: 'MAGISK-V17.3.ZIP',
      wizFlashMagisk: 'Magisk flashen',
      wizInstallLatest: 'Neueste von GitHub installieren',
      wizChecking: 'prüft…',
      wizCheckNewer: 'Auf neuere Version prüfen',
      wizOr: '— oder —',
      wizPickDifferentBuild: 'ANDEREN BUILD WÄHLEN (ARMv7)',
      wizCustomBinary: 'EIGENE REVOICE-SERVER-BINÄRDATEI (ARMv7)',
      wizInstallCustomBuild: 'Eigenen Build installieren',
      wizBuildLatestEmos: 'Mit der neuesten emOS-Version bauen',
      wizPickDifferentInit: 'ANDEREN INIT WÄHLEN',
      wizCustomInit: 'EIGENE INIT-BINÄRDATEI (AARCH64, STATISCH)',
      wizBuildWithInit: 'Mit diesem init bauen',
      wizFlashEmosBtn: 'emOS flashen',
      wizBootPartition: 'die Boot-Partition',
      wizNoEscrow: 'Diese Sitzung hat kein gesichertes Image — die in Schritt 3 heruntergeladene revoice-stock-boot-*.img auswählen.',
      wizRestoreEscrow: 'Gesichertes Boot-Image zurückschreiben',
      wizRingHeading: 'Bei diesem Start auf den Leuchtring achten',
      wizRingFillingBold: 'Füllt sich, dann weiß, dann verblassend',
      wizRingFillingRest: 'oben und im Netz. Fertig, etwa 30 Sekunden.',
      wizRingTwoSegBold: 'Zwei leuchtende Segmente oben, pulsierend',
      wizRingTwoSegRest: 'wartet auf das Netz. Normal, und der größte Teil des Starts.',
      wizRingAmberBold: 'Durchgehend bernsteinfarben',
      wizRingAmberRest: 'das Gerät stellt sein letztes funktionierendes Image selbst wieder her. In Ruhe lassen; es startet von allein neu.',
      wizRingRedBold: 'Rot und stehend',
      wizRingRedRest: 'eine Startstufe ist fehlgeschlagen. Reparierbar, siehe unten.',
      wizRingOrbitBold: 'Ein einzelnes Segment kreist über einem vollen blauen Ring, länger als eine Minute',
      wizRingOrbitRest: 'emOS ist nie gestartet. Das ist der Fall, der dich braucht.',
      wizNoPowerCycleBold: 'Wenn es nicht hochkommt, nicht immer wieder die Stromzufuhr trennen.',
      wizMuteButton: 'Stumm',
      wizCyanPattern: 'abwechselndes Cyan-Muster',
      wizRebootConnectConsole: 'Neu starten und Konsole verbinden',
      wizCancelStep: 'Schritt abbrechen',
      wizStepFailed: 'Dieser Schritt ist fehlgeschlagen',
      wizRetry: 'Erneut versuchen',
      wizReconnect: 'Neu verbinden',
      wizReconnectDevice: 'Gerät neu verbinden',
      wizDownloadDiagnostics: 'Diagnosepaket herunterladen',
      wizComplete: 'Einrichtung abgeschlossen. Das Gerät hat neu gestartet und findet den Controller über mDNS; es taucht innerhalb von etwa 30 Sekunden als wartendes Gerät im Dashboard auf.',
      wizDone: 'Fertig',
      wizOutput: 'Ausgabe',
      wizCopyLog: 'Protokoll kopieren',
      wizNoOutput: '— noch keine Ausgabe —',
      wizFlashBlurb(target) {
        return `Schreibt das gebaute Image nach ${target}. Dein gesichertes `
             + 'Image stellt sie in etwa zehn Sekunden wieder her und l\u00e4sst '
             + '/data unber\u00fchrt.';
      },
      wizRestoreBlurb(target, md5) {
        return `Setzt ${target} auf das in Schritt 3 gesicherte Image `
             + `zur\u00fcck${md5}. Wird danach gegen die Partition gepr\u00fcft, und `
             + '/data bleibt unangetastet \u2014 alles Installierte bleibt.';
      },
      wizNoPowerCyclePre: 'So kommst du nach TWRP: Strom abziehen, die Taste ',
      wizNoPowerCycleMid: ' gedr\u00fcckt halten und die Stromzufuhr herstellen, w\u00e4hrend sie gedr\u00fcckt bleibt \u2014 der Ring zeigt ein ',
      wizNoPowerCyclePost: ', sobald du in der Recovery bist. Hier neu verbinden und unten ',
      wizNoPowerCycleEnd: ' benutzen: etwa zehn Sekunden, und alles auf /data bleibt liegen. Ein Ger\u00e4t, das nicht startet, immer wieder vom Strom zu trennen, ist das, was aus einem reparierbaren Fall einen macht, f\u00fcr den das Geh\u00e4use aufgeht.',
      wizLatestOnGithub(v) { return `Neueste auf GitHub: ${v}`; },
      wizOutputLines(n) { return ` \u2014 ${n} Zeilen`; },
      wizMigrateDevice(id) { return `\u201e${id}" nach emOS \u00fcbernehmen (Einstellungen behalten)`; },
      wizDeleteDevice(id) { return `\u201e${id}" aus dem Controller l\u00f6schen`; },
      // -- WiFi panel --
      wifiScanForNetworks: 'Netzwerke suchen',
      wifiSsidPlaceholder: 'oben auswählen oder Netzwerknamen eintippen',
      wifiPassword: 'PASSWORT',
      wifiPassphrasePlaceholder: 'WPA-Passwort',
      wifiHide: 'verbergen',
      wifiShow: 'zeigen',
      wifiConnect: 'Verbinden',
      wifiSkip: 'Überspringen (schon verbunden)',
      wifiAbort: 'Einrichtung abbrechen',
      wifiNetworksFound(n) {
        return n === 1 ? 'ein Netzwerk gefunden' : `${n} Netzwerke gefunden`;
      },
