...
(function (global) {
  ...
  const STRINGS = {
    en: {
      ...
      wizClose: 'Close',
      wizResetState: 'Reset this device\u2019s Revoice state',
      wizResetStateSub: 'Removes the CA, token, remembered controller and console password this Echo was provisioned with. Use it when a device changes hands and the new controller answers "token mismatch". It keeps emOS, the firmware and the endpoint binaries.',
      wizResetStateGo: 'Clear credentials',
      wizResetDone: 'Cleared. The device dials again within seconds, presents no token, and appears as a pending device on this controller \u2014 approve it there.',
      wizAlreadyEmos: 'This Echo already runs emOS',
      wizAlreadyEmosSub: 'Skip to its serial console to put it on a different network — after a move, or when it changes hands. Nothing is read from the device or written to it, and the USB picker in step 1 would stay empty anyway: emOS has no adbd.',
      wizConsoleOnly: 'Console and WiFi only',
      wizSkippedToConsole: 'Skipping to the console — this device already runs emOS. Nothing has been read from it or written to it.',
      wizWipeFirst: 'Wipe data and cache first',
      wizWipeFirstOn: 'Erases /data and /cache in TWRP: WiFi, settings and anything installed. Cannot be undone.',
      wizWipeFirstOff: 'Off: the device keeps its data.',
      wizConnectConsole: 'Connect Console',
      wizRebootConnectConsole: 'Reboot and Connect Console',
      wizMigrateKeep: 'Migrate to emOS, keep its settings',
      wizKeepRecord: 'Re-provision, keep its record',
      wizThenWipes: 'Then wipes /data and /cache.',
      ...
      cfgSensitivitySub: 'wake confidence needed — raise it if ordinary speech wakes the Echo',
      ...
      devAirplayFlavourPending: 'the setting asks for the other one; it takes effect when the receiver next restarts',
      ...
      cfgAirplayNameSub: 'what THIS Echo is called in the AirPlay list. Blank uses the device label',
      cfgSpotifyNameSub: 'what THIS Echo is called in the Spotify app. Blank uses the device label',
      ...
      cfgJackBypass: 'Bypass protection on the jack',
      cfgJackBypassSub: 'off while something is plugged in — for an amplifier or powered speakers',
      cfgNoJackBypass: 'needs firmware that shapes its own audio and a device that detects the jack',
      cfgJackBypassNote: 'Speaker protection is voiced for the small driver inside the Echo, and it removes almost everything below 160 Hz: 28 dB at 31.5 Hz, 19 dB at 63 Hz. With a plug in the jack that driver is out of the signal path, so on a real amplifier the protection is only a filter, and the result sounds like midrange with no depth. Off by default, because it has not been confirmed on hardware that the internal driver is really silenced with a plug in — if it is not, this sends unfiltered bass to it. Turn it on with the external speakers switched off first and listen to the Echo itself: if you hear bass from it, switch this back off.',
    ...
    de: {
      ...
      wizClose: 'Schließen',
      wizResetState: 'Revoice-Zustand dieses Geräts zurücksetzen',
      wizResetStateSub: 'Entfernt Zertifizierungsstelle, Token, gemerkten Controller und Konsolenpasswort, mit denen dieses Echo eingerichtet wurde. Für den Fall, dass ein Gerät den Besitzer wechselt und der neue Controller mit „token mismatch" antwortet. emOS, Firmware und Endpunkt-Programme bleiben erhalten.',
      wizResetStateGo: 'Zugangsdaten löschen',
      wizResetDone: 'Gelöscht. Das Gerät wählt sich binnen Sekunden neu ein, legt keinen Token vor und erscheint auf diesem Controller als wartendes Gerät — dort freigeben.',
      wizAlreadyEmos: 'Auf diesem Echo läuft bereits emOS',
      wizAlreadyEmosSub: 'Direkt zur seriellen Konsole springen, um es in ein anderes Netz zu bringen — nach einem Umzug oder wenn es den Besitzer wechselt. Vom Gerät wird nichts gelesen und nichts darauf geschrieben, und die USB-Auswahl in Schritt 1 bliebe ohnehin leer: emOS hat kein adbd.',
      wizConsoleOnly: 'Nur Konsole und WLAN',
      wizSkippedToConsole: 'Springe zur Konsole — auf diesem Gerät läuft bereits emOS. Es wurde nichts davon gelesen und nichts darauf geschrieben.',
      wizWipeFirst: 'Vorher /data und /cache löschen',
      wizWipeFirstOn: 'Löscht /data und /cache in TWRP: WLAN, Einstellungen und alles Installierte. Nicht rückgängig zu machen.',
      wizWipeFirstOff: 'Aus: Das Gerät behält seine Daten.',
      wizConnectConsole: 'Konsole verbinden',
      wizRebootConnectConsole: 'Neu starten und Konsole verbinden',
      wizMigrateKeep: 'Auf emOS umstellen, Einstellungen behalten',
      wizKeepRecord: 'Neu einrichten, Eintrag behalten',
      wizThenWipes: 'Löscht danach /data und /cache.',
      ...
      cfgSensitivitySub: 'nötige Wachwort-Sicherheit — anheben, wenn gewöhnliches Reden den Echo weckt',
      ...
      devAirplayFlavourPending: 'die Einstellung verlangt die andere; sie greift beim nächsten Neustart des Empfängers',
      ...
      cfgAirplayNameSub: 'wie DIESER Echo in der AirPlay-Liste heißt. Leer nimmt die Gerätebezeichnung',
      cfgSpotifyNameSub: 'wie DIESER Echo in der Spotify-App heißt. Leer nimmt die Gerätebezeichnung',
      ...
      cfgJackBypass: 'Schutz am Klinkenausgang aussetzen',
      cfgJackBypassSub: 'aus, solange etwas eingesteckt ist — für Verstärker oder Aktivboxen',
      cfgNoJackBypass: 'braucht Firmware, die ihren Klang selbst formt, und ein Gerät mit Klinkenerkennung',
      cfgJackBypassNote: 'Der Lautsprecherschutz ist auf den kleinen Treiber im Echo abgestimmt und nimmt fast alles unter 160 Hz heraus: 28 dB bei 31,5 Hz, 19 dB bei 63 Hz. Mit einem Stecker in der Buchse liegt dieser Treiber gar nicht mehr im Signalweg, an einem richtigen Verstärker ist der Schutz also nur noch ein Filter — und das klingt nach Mitten ohne Tiefe. Standardmäßig aus, weil an der Hardware nicht bestätigt ist, dass der interne Treiber mit Stecker wirklich stumm ist; falls nicht, bekommt er ungefilterten Bass ab. Zum Einschalten zuerst die externen Boxen ausschalten und auf das Echo selbst hören: kommt daraus Bass, diese Einstellung wieder ausschalten.',
