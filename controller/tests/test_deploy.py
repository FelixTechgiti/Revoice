...
def test_ci_runs_every_dashboard_test():
    """
    A `.test.mjs` that CI does not run is a test nobody runs.

    This used to check that every file was NAMED in ci.yml, because the suite
    was a list of explicit `node <file>` lines. The reason for the list was
    real — `node --test tests/` picks up files that are not tests and reports
    differently — but the list itself was the defect it kept catching: adding
    a test was two steps, the second invisible when missed, and this assertion
    was the only thing standing between a new test and never running again.
    It caught `update_badge.test.mjs` (#255), and by 2026-09-25 the list held
    31 names.

    The step now iterates the directory and runs each file the same way the
    list did — one `node <file>` per test, so the objection above still does
    not apply — and the drift is gone rather than guarded. What is left to
    check is that it stayed that way: a future edit back to a hand-written
    list would pass silently under a test that only counted names.
    """
    ...
    # Just this step: from its name to the next one at the same indentation.
    step = ci[ci.index("Run dashboard tests"):]
    nxt = step.find("\n      - name:")
    if nxt > 0:
        step = step[:nxt]

    assert "controller/tests/*.test.mjs" in step, (
        "The Dashboard logic tests step no longer iterates the directory. A "
        "hand-written list drifts: a test added without a line there never "
        "runs, and the gap looks like nothing."
    )
    named = [t for t in tests if f"controller/tests/{t}" in step]
    assert not named, (
        "These are named individually in ci.yml on top of the glob:\n  "
        + "\n  ".join(named)
        + "\n\nThe glob already runs them; a second mention is a list "
          "growing back."
    )
