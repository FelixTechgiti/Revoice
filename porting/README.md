# Porting: profiling an Echo Revoice doesn't support yet
...
They don't make the device work with Revoice. They collect what a port needs.
...
- **Don't run these on a device running Revoice.** Its server holds the audio
...
- **Mics (opt-in, `--mics`).** Stops the service holding the microphones
  ...
  - Stock `tinycap` can't record every mic format; the Dot 2's packed 24-bit
    ...
    `porting/pcm_capture/build.sh` (needs the `revoice-compiler` Docker
