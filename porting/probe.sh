...
#   - Refuses to run beside Revoice's own server, which holds the PCMs too.
set -eu
...
OUT="${1:-./revoice-probe-$model-$(date +%Y%m%d-%H%M%S)}"
mkdir -p "$OUT"
P="$OUT/probe.txt"; : > "$P"
say() { printf '%s\n' "$*" >&2; }
sec() { printf '\n===== %s\n' "$1" >> "$P"; }

# Revoice's server holds the PCMs; stock mediaserver is what we want to watch.
if dev "ls -l /proc/*/exe" | grep -q '/data/local/bin/server'; then
  say "Revoice's server is running on this device. Stop it first (stop revoice),"
...
{ echo "Revoice active probe"; echo "probe.sh format 1"; echo "host date: $(date -u +%Y-%m-%dT%H:%M:%SZ)"
